...
void LYSimPrimaryGeneratorAction::RandomizePosition()
{
  ...
  for (int i = 0; i < np; ++i)
  {
    ...
    pos->SetHalfZ(z / 2);
    // pos->SetPosRot2(G4ThreeVector(0., 0, -1));//normal setup
    // pos->SetCentreCoords(G4ThreeVector(x, 0, y));//normal setup
    pos->SetPosRot2(G4ThreeVector(0., -1, 0));
    pos->SetCentreCoords(G4ThreeVector(0, 0, 0));//test fiber
