...
LYSimDebugGeneratorAction::LYSimDebugGeneratorAction() : particleSource(new G4GeneralParticleSource()),
...
void LYSimDebugGeneratorAction::GeneratePrimaries(G4Event *anEvent)
{
...
void LYSimDebugGeneratorAction::SetParticlePosition(const G4ThreeVector &v)
{
  particleSource->GetCurrentSource()->GetPosDist()->SetCentreCoords(v);
}

void LYSimDebugGeneratorAction::SetParticleDirection(const G4ThreeVector &v)
{
  G4SPSAngDistribution *ang = particleSource->GetCurrentSource()->GetAngDist();
...
void LYSimDebugGeneratorMessenger::SetNewValue(G4UIcommand *command, G4String val)
{
