...
MakeS_Absorbing()
{
  ...
  const unsigned nentries       = 2;
  double phoE[nentries]         = {1.0*eV, 6.0*eV};
  double reflectivity[nentries] = {0.01, 0.01};
  ...
  table->AddProperty("EFFICIENCY", phoE, EFFICIENCY,2);
...
MakeS_NoAbsorbing()
{
  ...
  // Absorbing surface
  //////////////////////////////////
  G4OpticalSurface* surface = new G4OpticalSurface( "NoAbsorbingOpSurface" );
  surface->SetType( dielectric_metal );
  surface->SetFinish( ground );
  surface->SetModel( unified );
...
MakeS_RoughInterface( const double alpha )
{
  ...
  //double phoE[2]          = {1.0*eV, 6.0*eV};
  //table->AddProperty("EFFICIENCY", phoE, reflectivity,2);
  surface->SetMaterialPropertiesTable( table );
